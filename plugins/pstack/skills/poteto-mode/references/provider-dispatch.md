...
Claude keeps `--setting-sources project`. Before executable lookup, the launcher structurally reads `~/.claude/settings.json` and, when present, projects only `ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN`, and `ANTHROPIC_API_KEY` from its `env` object. That complete projection replaces all inherited values for those keys. A custom base URL requires exactly one of the two credential keys; malformed or ambiguous settings fail before preflight. The same frozen environment is used for lookup, preflight, and execution. Receipts record only the source and imported/replaced key names.
